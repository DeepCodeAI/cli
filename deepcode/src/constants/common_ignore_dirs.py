...
# Note:
#    - regex rules as per https://docs.python.org/3/library/re.html
#    - Fixes for earlier version (TODO may need to be ported to VSCode too?):
#      - dots must be escaped to really test for dots instead of a single-character wildcard
#      - TODO a leading and trailing slash is needed to avoid partial directory name matches (bar/ unintentionally matches foobar/)
#        This may be something that better be post-processed instead of encoding it in the directory string
COMMON_IGNORE_DIRS = []
# COMMON_IGNORE_DIRS = [
#     "[Aa][Rr][Mm]/",
#     "[Aa][Rr][Mm]64/",
